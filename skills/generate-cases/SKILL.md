...
- **IMPORTANT: NEVER GENERATE test or suite IDs of ANY kind**:
  ...
  - Testomat.io IDs are server-generated and will be assigned automatically when the user runs `check-tests push` with the `--update-ids` option (or via the `sync-cases` skill). No need to add IDs at test cases generation time.
